...
// Caché local en memoria para la Semana 8.
// Estrategia cache-aside con tiempo de vida de 60 segundos.
const cacheRegistros = new Map();
const CACHE_TTL_MS = 60 * 1000;

function obtenerCache(clave) {
  const entrada = cacheRegistros.get(clave);
...
function guardarCache(clave, valor) {
  cacheRegistros.set(clave, {
    valor,
    expiraEn: Date.now() + CACHE_TTL_MS
  });
}

function invalidarCacheRegistros() {
  cacheRegistros.clear();
  console.log("[CACHE] Caché de registros invalidada");
}


// Cola asíncrona para procesar lotes de sincronización.
const colaSincronizacion = [];
const trabajosSincronizacion = new Map();
...
function encolarLoteSincronizacion(trabajo) {
  colaSincronizacion.push(trabajo);
  trabajosSincronizacion.set(trabajo.id_lote, trabajo);

  console.log(
    `[COLA] Lote ${trabajo.id_lote} agregado. Pendientes: ${colaSincronizacion.length}`
  );

  // El worker comienza después de devolver la respuesta HTTP.
  setImmediate(procesarColaSincronizacion);
}
...
async function procesarColaSincronizacion() {
  ...
  while (colaSincronizacion.length > 0) {
    const trabajo = colaSincronizacion.shift();

    trabajo.estado = "procesando";
    trabajo.fecha_inicio_proceso = new Date().toISOString();

    console.log(`[WORKER] Procesando lote ${trabajo.id_lote}`);

    try {
      // Simula validación, transformación y sincronización en segundo plano.
      await esperar(1500);

      trabajo.estado = "completado";
      trabajo.registros_procesados = trabajo.registros.length;
      trabajo.fecha_fin = new Date().toISOString();

      console.log(
        `[WORKER] Lote ${trabajo.id_lote} completado: ` +
        `${trabajo.registros_procesados} registros`
      );
    } catch (error) {
      trabajo.estado = "error";
      trabajo.error = "No fue posible procesar el lote";
      trabajo.fecha_fin = new Date().toISOString();

      console.error(`[WORKER] Error en lote ${trabajo.id_lote}`);
    }
  }
...
// Catálogos relacionados utilizados por REGISTRO_BOTANICO.
const plantas = [
  {
    id_planta: 1,
    nombre_cientifico: "Ilex guayusa",
    nombre_local_principal: "Guayusa",
    estado_taxonomico: "validada"
  }
];
...
const comunidades = [
  {
    id_comunidad: 1,
    nombre: "Comunidad Amazónica Demo",
    provincia: "Pastaza",
    idioma_predominante: "Kichwa"
  }
];

// Diagnóstico del riesgo N+1:
// realiza tres búsquedas por cada registro.
function diagnosticarNMasUno(registrosPagina) {
  let consultasRelaciones = 0;

  registrosPagina.forEach((registro) => {
    plantas.find((p) => p.id_planta === registro.id_planta);
    consultasRelaciones += 1;

    usuarios.find((u) => u.id_usuario === registro.id_usuario);
    consultasRelaciones += 1;

    comunidades.find((c) => c.id_comunidad === registro.id_comunidad);
    consultasRelaciones += 1;
  });

  return {
    estrategia: "N+1: búsqueda individual por cada registro",
    registros_procesados: registrosPagina.length,
    consultas_relaciones: consultasRelaciones
  };
}

// Corrección mediante carga agrupada equivalente a eager loading.
// Las relaciones se preparan una vez y luego se reutilizan.
function cargarRelacionesEager(registrosPagina) {
  const idsPlantas = new Set(registrosPagina.map((r) => r.id_planta));
  const idsUsuarios = new Set(registrosPagina.map((r) => r.id_usuario));
  const idsComunidades = new Set(registrosPagina.map((r) => r.id_comunidad));

  const mapaPlantas = new Map(
    plantas
      .filter((p) => idsPlantas.has(p.id_planta))
      .map((p) => [p.id_planta, p])
  );

  const mapaUsuarios = new Map(
    usuarios
      .filter((u) => idsUsuarios.has(u.id_usuario))
      .map((u) => [u.id_usuario, u])
  );

  const mapaComunidades = new Map(
    comunidades
      .filter((c) => idsComunidades.has(c.id_comunidad))
      .map((c) => [c.id_comunidad, c])
  );

  const registrosEnriquecidos = registrosPagina.map((registro) => {
    const usuario = mapaUsuarios.get(registro.id_usuario);

    return {
      ...registro,
      planta: mapaPlantas.get(registro.id_planta) || null,
      usuario: usuario ? ocultarPassword(usuario) : null,
      comunidad: mapaComunidades.get(registro.id_comunidad) || null
    };
  });
  ...
  return {
    ...
    diagnostico: {
      estrategia: "Eager loading mediante carga agrupada",
      registros_procesados: registrosPagina.length,
      consultas_relaciones: 3
    }
  };
}
...
app.get("/api/registros", autenticar, (req, res) => {
  const inicioMedicion = process.hrtime.bigint();
  const pagina = Number(req.query.pagina || 1);
  const limite = Number(req.query.limite || 10);

  // La clave considera rol, usuario y paginación para evitar mezclar respuestas.
  const claveCache =
    `registros:${req.usuario.rol}:${req.usuario.id_usuario}:${pagina}:${limite}`;

  // Estrategia cache-aside: primero se consulta la caché.
  const datosCache = obtenerCache(claveCache);

  if (datosCache) {
    const finMedicion = process.hrtime.bigint();
    const tiempoMs = Number(finMedicion - inicioMedicion) / 1_000_000;

    console.log(`[CACHE HIT] ${claveCache} - ${tiempoMs.toFixed(3)} ms`);
    ...
    return respuestaExito(
      ...
      200,
      {
        ...
        optimizacion: {
          cache: "HIT",
          ttl_segundos: CACHE_TTL_MS / 1000,
          tiempo_procesamiento_ms: Number(tiempoMs.toFixed(3))
        }
      },
      "Registros consultados correctamente desde caché"
    );
  }

  // Cache miss: se obtiene y procesa la información original.
  let visibles = registros.filter((r) => !r.eliminado);

  if (req.usuario.rol === "comunitario") {
    visibles = visibles.filter(
      (r) => r.id_usuario === req.usuario.id_usuario
    );
  }

  const inicioPagina = (pagina - 1) * limite;
  const datos = visibles.slice(inicioPagina, inicioPagina + limite);

  // Comparación entre el riesgo N+1 y la carga agrupada.
  const diagnosticoNMasUno = diagnosticarNMasUno(datos);
  const cargaEager = cargarRelacionesEager(datos);

  const resultado = {
    registros: cargaEager.registros,
    paginacion: {
      paginaActual: pagina,
      limite,
    ...
    optimizacion_carga: {
      antes: diagnosticoNMasUno,
      despues: cargaEager.diagnostico,
      estrategia_seleccionada: "eager loading mediante carga agrupada",
      justificacion:
        "El listado necesita planta, usuario y comunidad; se cargan una sola vez para evitar búsquedas repetidas por cada registro."
    }
  };

  // Si no estaba en caché, se guarda durante 60 segundos.
  guardarCache(claveCache, resultado);

  const finMedicion = process.hrtime.bigint();
  const tiempoMs = Number(finMedicion - inicioMedicion) / 1_000_000;

  console.log(`[CACHE MISS] ${claveCache} - ${tiempoMs.toFixed(3)} ms`);
  ...
  return respuestaExito(
    ...
    "Registros consultados y almacenados en caché"
  );
});
...
// Recibe un lote y responde inmediatamente con HTTP 202.
app.post(
  "/api/sync/lotes",
  autenticar,
  autorizarRoles("comunitario", "tecnico", "admin"),
  (req, res) => {
    const registrosLote = req.body.registros;

    if (!Array.isArray(registrosLote) || registrosLote.length === 0) {
      return respuestaError(
        res,
        422,
        [{
          campo: "registros",
          mensaje: "Debe enviar al menos un registro"
        }],
        "Datos inválidos"
      );
    }
    ...
    const trabajo = {
      ...
      id_usuario: req.usuario.id_usuario,
      estado: "en_cola",
      total_registros: registrosLote.length,
      registros_procesados: 0,
      registros: registrosLote,
      fecha_recepcion: new Date().toISOString()
    };

    encolarLoteSincronizacion(trabajo);

    return respuestaExito(
      res,
      202,
      {
        id_lote: trabajo.id_lote,
        estado: trabajo.estado,
        total_registros: trabajo.total_registros,
        mensaje_worker:
          "El lote fue aceptado y será procesado en segundo plano"
      },
      "Lote de sincronización recibido"
    );
  }
);

// Consulta el estado del trabajo procesado por el worker.
app.get("/api/sync/lotes/:id", autenticar, (req, res) => {
  const trabajo = trabajosSincronizacion.get(req.params.id);

  if (!trabajo) {
    return respuestaError(
      res,
      404,
      [{ campo: "id", mensaje: "Lote no encontrado" }],
      "Recurso no encontrado"
    );
  }
  ...
  if (
    ...
    trabajo.id_usuario !== req.usuario.id_usuario
  ) {
    return respuestaError(
      res,
      403,
      [{ campo: "usuario", mensaje: "No puede consultar este lote" }],
      "No autorizado"
    );
  }
  ...
  return respuestaExito(
    ...
    200,
    {
      ...
      fecha_inicio_proceso: trabajo.fecha_inicio_proceso || null,
      fecha_fin: trabajo.fecha_fin || null
    },
    "Estado del lote consultado"
  );
});
